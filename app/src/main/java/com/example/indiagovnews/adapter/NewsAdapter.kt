    ...
    class NewsViewHolder(
        ...
        fun bind(article: NewsArticle) {
            binding.apply {
                ...
                // Set sentiment color if available
                article.sentiment?.let { sentiment ->
                    sentimentIndicator.setBackgroundColor(when {
                        sentiment > 0.2 -> 0xFF4CAF50.toInt() // Positive - Green
                        sentiment < -0.2 -> 0xFFF44336.toInt() // Negative - Red
                        else -> 0xFFFFEB3B.toInt() // Neutral - Yellow
                    })
